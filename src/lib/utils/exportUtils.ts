...
`;
    ...
    const now = new Date();
    const ts = `${now.toISOString().split('T')[0]}_${now.toTimeString().split(' ')[0].replace(/:/g, '-')}`;
    const filename = `${survey.name?.toLowerCase().replace(/[^a-z0-9]/g, '-') || survey.id}-complete-export-${ts}.zip`;
...
export const exportSurveyWithImages = async (survey: Survey | null): Promise<void> => {
  ...
  try {
    ...
    const tsImg = `${nowImg.toISOString().split('T')[0]}_${nowImg.toTimeString().split(' ')[0].replace(/:/g, '-')}`;
    const filename = `${survey.name?.toLowerCase().replace(/[^a-z0-9]/g, '-') || survey.id}-with-images-${tsImg}.zip`;
...
`;
  ...
  const zipBlob = await zip.generateAsync({ type: 'blob' });
  const nowPkg = new Date();
  const tsPkg = `${nowPkg.toISOString().split('T')[0]}_${nowPkg.toTimeString().split(' ')[0].replace(/:/g, '-')}`;
  const filename = `${survey.name?.toLowerCase().replace(/[^a-z0-9]/g, '-') || survey.id}-${tsPkg}.zip`;
